		...
		if (currentUser.getUserRights()) {
			...
			manageUsers.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent useravt) {
					...
					ComboBox<String> userSelect = new ComboBox<String>();
					userSelect.getItems().addAll("Admin", "User");
					userSelect.setValue("User");
					GridPane userInner = new GridPane();
					userInner.setHgap(10);
					userInner.setVgap(10);
					...
					addUser.setTooltip(new Tooltip("Enter username and password for new user"));
							...
							if (userName.getText().equals("") || userPassword.getText().equals("")) {
								...
								if (!doubleEntry) {
									...
									try {
										...
										alert.setContentText("New user '" + name + "' has been created successfully.");
										alert.showAndWait();
									} catch (IOException i) {
										logger.log(Level.SEVERE, "" + i);
									}
					...
					grid.add(new Label("User Type"), 2, 0);
					grid.add(userName, 0, 1);
					grid.add(userPassword, 1, 1);
					grid.add(userSelect, 2, 1);
					grid.add(addUser, 3, 1);
					grid.add(new Label("Existing users"), 0, 2);
					grid.add(userInnerScrl, 0, 3, 4, 1);
